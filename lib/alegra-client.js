...
class AlegraClient {
     ...
     * @param {String} invoiceData.paymentMethod - Método de pago (requerido si paymentForm es CASH y facturación electrónica 2.1 está activa)
     * @param {String} invoiceData.paymentForm - Forma de pago: CASH o CREDIT (requerido para facturación electrónica 2.1)
     * @param {Object} invoiceData.stamp - Objeto para factura electrónica {generateStamp: true}
     * @param {String} invoiceData.type - Tipo de factura (default: NATIONAL)
     */
    async createInvoice(invoiceData) {
        // Agregar configuración para facturación electrónica Colombia por defecto si no está presente
        const invoicePayload = {
            ...invoiceData,
            type: invoiceData.type || 'NATIONAL',
            paymentForm: invoiceData.paymentForm || 'CREDIT',
            stamp: invoiceData.stamp || { generateStamp: true }
        };

        // Agregar paymentMethod solo si paymentForm es CASH
        if (invoicePayload.paymentForm === 'CASH' && !invoicePayload.paymentMethod) {
            invoicePayload.paymentMethod = '1'; // Efectivo por defecto
        }

        return this._request('POST', '/invoices', invoicePayload);
    }
